...
(function ($) {
    ...
    var appmyil = {
        ...
        serviceImageAnim: function () {
            const images = ['.tm-service-image-1', '.tm-service-image-2', '.tm-service-image-3']
            let active = 'tm-service-image-1'
            function toggle() {
                let currentIndex = images.indexOf(active)
                active = images[currentIndex + 1] || images[0]
            }
            setInterval(function () {
                toggle()
                $(active).addClass('is-visible')
                images.filter(image => image !== active).forEach(image => $(image).removeClass('is-visible'))
            }, 5000);
        },
