...
class LandingController extends Controller
{
    ...
    {
        $commitees = Http::get(config('app.api_url').'/api/cabinets?populate=commitees.photo,commitees.gif,commitees.division&filters[year][$eq]='.Carbon::now()->year);
        ...
        $data = count(json_decode($commitees->body())->data) > 0 ? json_decode($commitees->body())->data : json_decode(Http::get(config('app.api_url').'/api/cabinets?populate=commitees.photo,commitees.gif,commitees.division&filters[year][$eq]='.Carbon::now()->subYear()->year)->body())->data;
        ...
        $data = collect($data)->map(function ($item) {
            $commitees = collect($item->attributes->commitees->data)->map(function ($commitee) use ($item) {
                return (object) [
                    ...
                    'gif' => $commitee->attributes->gif->data !== null ? config('app.api_url').$commitee->attributes->gif->data->attributes->url : '',
                ];
            });
    ...
    {
        if ($request->has('year')) {
            $commitees = Http::get(config('app.api_url').'/api/commitees?populate=photo,gif,cabinet,division&filters[cabinet][year][$eq]='.$request->year.'&sort[1]=division.priority&pagination[page]=1&pagination[pageSize]=40');
            $data['commitees'] = collect(json_decode($commitees->body())->data)->map(function ($item) {
                return (object) [
                    ...
                    'gif' => $item->attributes->gif->data !== null ? config('app.api_url').$item->attributes->gif->data->attributes->url : '',
                ];
            });
        } else {
            $commitees = Http::get(config('app.api_url').'/api/commitees?populate=photo,gif,cabinet,division&filters[cabinet][year][$eq]='.Carbon::now()->year.'&sort[1]=division.priority&pagination[page]=1&pagination[pageSize]=40');
    ...
    public function contactUs(Request $request)
    {
        ...
        try {
            Mail::to('infinite@student.uny.ac.id')->send(new ContactUs($data));
