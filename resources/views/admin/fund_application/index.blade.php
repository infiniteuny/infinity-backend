...
@section('js')
    ...
    <script type="text/javascript">
        $(function() {
            var table = $('.fund_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.fund-application.index') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex'
                    },
                    ...
                    },
                    {
                        ...
                        render: function(data) {
                            return `
                                <form action="{{ url('admin/fund-application') }}/${data.id}/download/student-id-card" method="POST">
                                    @csrf
                    ...
                    },
                    {
                        ...
                        render: function(data) {
                            return `
                                <form action="{{ url('admin/fund-application') }}/${data.id}/download/letter-of-acceptance" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-success btn-xs"><i class="fa fa-download"></i> </button>
                                </form>
                            `;
                    ...
                    },
                    {
                        ...
                        render: function(data) {
                            return `
                                <form action="{{ url('admin/fund-application') }}/${data.id}/download/budget-plan" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-success btn-xs"><i class="fa fa-download"></i> </button>
                                </form>
                            `;
                    ...
                    },
                    {
                        ...
                        render: function(data) {
                            if (data.status == 'waiting') {
                                return `
                                <form action="{{ url('admin/fund-application') }}/${data.id}" method="POST">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        function goTo(id) {
            var id = id;
            var url = '{{ route('admin.fund-application.edit', ':id') }}';
        ...
        function accept(id) {
            var id = id;
            var url = '{{ route('admin.fund-application.accept', ':id') }}';
            url = url.replace(':id', id);
            window.location.href = url;
        }
        ...
        function reject(id) {
            var id = id;
            var url = '{{ route('admin.fund-application.reject', ':id') }}';
            url = url.replace(':id', id);
            window.location.href = url;
        }
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.fund-application.store') }}" method="POST"
