...
export function setup() {
    ...
    group('Setup - Independent Resources', () => {
        ...
        // Competition Instances (multipart with http.file)
        res = trackedRequest(
        ...
        res = trackedRequest(
            ...
            JSON.stringify({ year: 9000, is_active: false }),
            JSON_HEADERS, 'setup'
        );
        ...
        res = trackedRequest(
            ...
            {
                name: `k6_setup_persona${uid()}`,
                priority: '1',
                description: 'k6 setup persona',
                logo: http.file(IMAGE_BIN, 'test-image.jpg', 'image/jpeg'),
            },
            { ...AUTH_HEADERS, Accept: 'application/json' },
            'setup'
        ...
        res = trackedRequest(
            ...
            JSON.stringify({
                name: `k6_setup_leader${uid()}`,
                username: `k6sl${leaderUid}`,
                email_address: `k6sl${leaderUid}@test.com`,
                phone_number: `081${leaderUid.slice(0, 10)}`,
                student_id: `11111${leaderUid.slice(0, 10)}`,
        ...
        // Users - member user (separate from leader, used for member endpoints)
        const memberUid = uid().replace(/_/g, '');
        res = trackedRequest(
            'POST', `${BASE_URL}/v1/users`,
            JSON.stringify({
                name: `k6_setup_member${uid()}`,
                username: `k6sm${memberUid}`,
                email_address: `k6sm${memberUid}@test.com`,
                phone_number: `082${memberUid.slice(0, 10)}`,
                student_id: `22222${memberUid.slice(0, 10)}`,
                major_id: ids.majorId,
                links: {},
                is_member: false,
                is_extraordinary: false,
        ...
        ids.memberUserId = extractId(res) || 'test-member-id';
        ...
        res = trackedRequest(
            ...
            {
                name: `k6_setup_testimonial${uid()}`,
                position: 'k6 setup tester',
                content: 'k6 setup content',
...
export function teardown(ids) {
    ...
    group('Teardown - Cleanup', () => {
        // Deactivate active resources before deleting (CannotDeactivateActive rule)
        if (ids.coreTeamId && ids.coreTeamId !== 'test-ct-id') {
            trackedRequest('PUT', `${BASE_URL}/v1/core-teams/${ids.coreTeamId}`, JSON.stringify({ is_active: false }), JSON_HEADERS, 'teardown');
        }
        if (ids.communityGroupAdminId && ids.communityGroupAdminId !== 'test-cga-id') {
            trackedRequest('PUT', `${BASE_URL}/v1/community-group-admins/${ids.communityGroupAdminId}`, JSON.stringify({ is_active: false }), JSON_HEADERS, 'teardown');
        }

        const deletions = [
            ids.testimonialId !== 'test-testimonial-id' ? `/v1/testimonials/${ids.testimonialId}` : null,
            ids.projectGalleryId !== 'test-pg-id' ? `/v1/project-galleries/${ids.projectGalleryId}` : null,
            ids.teamId !== 'test-team-id' ? `/v1/teams/${ids.teamId}` : null,
            ids.personaId !== 'test-persona-id' ? `/v1/personas/${ids.personaId}` : null,
            ids.communityGroupId !== 'test-cg-id' ? `/v1/community-groups/${ids.communityGroupId}` : null,
            ids.communityGroupAdminId !== 'test-cga-id' ? `/v1/community-group-admins/${ids.communityGroupAdminId}` : null,
            ids.memberUserId !== 'test-member-id' ? `/v1/users/${ids.memberUserId}` : null,
            ids.userId !== 'test-user-id' ? `/v1/users/${ids.userId}` : null,
            ids.groupId !== 'test-group-id' ? `/v1/groups/${ids.groupId}` : null,
            ids.permissionId !== 'test-perm-id' ? `/v1/permissions/${ids.permissionId}` : null,
            ids.coreTeamId !== 'test-ct-id' ? `/v1/core-teams/${ids.coreTeamId}` : null,
            ids.coreTeamDivisionId !== 'test-ctd-id' ? `/v1/core-team-divisions/${ids.coreTeamDivisionId}` : null,
            ids.competitionInstanceId !== 'test-ci-id' ? `/v1/competition-instances/${ids.competitionInstanceId}` : null,
            ids.competitionId !== 'test-comp-id' ? `/v1/competitions/${ids.competitionId}` : null,
            ids.competitionTeamTypeId !== 'test-ctt-id' ? `/v1/competition-team-types/${ids.competitionTeamTypeId}` : null,
            ids.competitionTimeRangeId !== 'test-ctr-id' ? `/v1/competition-time-ranges/${ids.competitionTimeRangeId}` : null,
            ids.competitionScaleId !== 'test-cs-id' ? `/v1/competition-scales/${ids.competitionScaleId}` : null,
            ids.competitionRankId !== 'test-cr-id' ? `/v1/competition-ranks/${ids.competitionRankId}` : null,
            ids.competitionOutputId !== 'test-co-id' ? `/v1/competition-outputs/${ids.competitionOutputId}` : null,
            ids.competitionOrganizerTypeId !== 'test-cot-id' ? `/v1/competition-organizer-types/${ids.competitionOrganizerTypeId}` : null,
            ids.majorId !== 'test-major-id' ? `/v1/majors/${ids.majorId}` : null,
            ids.facultyId !== 'test-faculty-id' ? `/v1/faculties/${ids.facultyId}` : null,
            ids.degreeId !== 'test-degree-id' ? `/v1/degrees/${ids.degreeId}` : null,
        ];
